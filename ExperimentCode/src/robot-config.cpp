...
int rc_auto_loop_function_Controller1() {
  ...
  while(true) {
    if(RemoteControlCodeEnabled) {
      ...
      // Tank drive with tolerance for better straight driving
      int drivetrainLeftSideSpeed = Controller1.Axis3.position();
      int drivetrainRightSideSpeed = Controller1.Axis2.position();

      if (drivetrainLeftSideSpeed < drivetrainRightSideSpeed + 25 && drivetrainLeftSideSpeed > drivetrainRightSideSpeed - 25) {
        drivetrainLeftSideSpeed += drivetrainRightSideSpeed;
        drivetrainRightSideSpeed += drivetrainLeftSideSpeed;
        drivetrainLeftSideSpeed /= 2;
        drivetrainRightSideSpeed /= 2;
      }
      ...
      if (!ArmReset) {
        if (Controller1.ButtonR1.pressing() && rotationSens.position(degrees) > 200) {
          // Moves arm forward
          ...
          Controller1RightShoulderControlMotorsStopped = false;
        } else if (!Controller1.ButtonR1.pressing() && (rotationSens.position(degrees) < 307 && ArmSet == true)) {
          armMotor.setVelocity(60, percent);
          armMotor.spin(forward);
          Controller1RightShoulderControlMotorsStopped = false;
        } else if (!Controller1.ButtonR1.pressing() && (rotationSens.position(degrees) > 323 && ArmSet == true)) {
          // Move arm forward to set position
